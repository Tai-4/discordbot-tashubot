...
Thread.new do
  ...
  self_introduction_channel = @bot.channel(self_introduction_channel_id, 784700980381351936) # Cacheを使用して自己紹介チャンネルのオブジェクトを生成。[引数: (チャンネルID, サーバーID) ]

  Thread.new do
    loop { message_list << self_introduction_channel.await!.message } # Message(Object)が戻り値。messageメソッドを使用しない場合は、Discordrb::Events::MessageEventというクラスのインスタンスが返される。
  end
  ...
  loop do
    ...
    delete_messages_ids << @bot.send_message(self_introduction_channel_id, "#{delete_messages_authors_mention.join(" ")} 自己紹介以外のメッセージは禁止されています！").id
...
@bot.command :dem do |event|
  ...
  if (1..100).include?(number)
    ...
    message_data = Discordrb::API::Channel.messages(@bot.token, event.channel.id, number, event_message_id)
    message_ids = JSON.parse(message_data).map { |data| data["id"]}
    if message_ids.size == 100
      Discordrb::API::Channel.delete_message(@bot.token, event.channel.id, event_message_id)
      Discordrb::API::Channel.bulk_delete_messages(@bot.token, event.channel.id, message_ids)
    else
